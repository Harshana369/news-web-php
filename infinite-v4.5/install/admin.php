...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        $stmt = $connection->prepare("INSERT INTO users (`id`,`username`,`slug`,`email`,`password`,`role_id`,`user_type`,`status`,`last_seen`,`session_token`,`reset_token`,`created_at`) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("issssisissss", $userId, $adminUsername, $slug, $adminEmail, $adminPasswordHash, $userRoleId, $userType, $userStatus, $date, $sessionToken, $resetToken, $date);
        $stmt->execute();
