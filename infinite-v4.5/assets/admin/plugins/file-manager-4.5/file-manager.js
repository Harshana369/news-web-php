    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.result == 1) {
                document.getElementById("image_file_upload_response").innerHTML = response.content;
            }
        }
    });
}
            ...
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        setTimeout(function () {
                            if (response.result == 1) {
                                $("#image_file_upload_response").append(response.content);
                            }
                        }, 100);
                    }
                });
...
function refresh_files() {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.result) {
                document.getElementById("file_upload_response").innerHTML = response.content;
            }
        }
    });
}
            ...
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.result) {
                            setTimeout(function () {
                                $("#file_upload_response").append(response.content);
                            }, 100);
                        }
                    }
                });
...
$(document).on('input', '#input_search_file', function () {
    ...
    $.ajax({
        ...
        data: data,
        success: function (response) {
            if (search.length > 1 && response.result) {
                document.getElementById("file_upload_response").innerHTML = response.content;
            } else {
                refresh_files();
            }
