...
class GalleryModel extends BaseModel
{
            ...
            for ($i = 0; $i < $fileCount; $i++) {
                if (isset($_FILES['files']['name'])) {
                    ...
                    if (isset($tmpFilePath)) {
                        ...
                        $newName = 'img_temp_' . generateToken(true) . '.' . $ext;
    ...
    {
        $this->builder->select('gallery_images.*, (SELECT name FROM gallery_albums WHERE id = gallery_images.album_id) AS album_name, (SELECT name FROM gallery_categories WHERE id = gallery_images.category_id) AS category_name');
    ...
    {
        return $this->builder->where('album_id', clrNum($albumId))->orderBy('id', 'DESC')->get()->getResult();
    }
    ...
    {
        $row = $this->builder->where('album_id', clrNum($album_id))->where('is_album_cover', 1)->orderBy('id', 'DESC')->get()->getRow();
        if (empty($row)) {
            $row = $this->builder->where('album_id', clrNum($album_id))->orderBy('id', 'DESC')->get()->getRow();
        }
        return $row;
    }
    ...
    public function editAlbum($id)
    {
        ...
        return $this->builderAlbums->where('id', clrNum($id))->update($data);
    }

    //get album
    public function getAlbum($id)
    {
        return $this->builderAlbums->where('id', clrNum($id))->get()->getRow();
    }
    ...
    {
        return $this->builderAlbums->where('lang_id', clrNum($this->activeLangId))->get()->getResult();
    }

    //get albums by lang
    public function getAlbumsByLang($langId)
    {
        return $this->builderAlbums->where('lang_id', clrNum($langId))->get()->getResult();
    }
    ...
    public function editCategory($id)
    {
        ...
        return $this->builderCategories->where('id', clrNum($id))->update($data);
    }
    ...
    {
        return $this->builderCategories->where('album_id', clrNum($albumId))->get()->getResult();
    }

    //get album category count
    public function getCategoryCountByAlbum($albumId)
    {
        return $this->builderCategories->where('album_id', clrNum($albumId))->where('lang_id', $this->activeLangId)->countAllResults();
    }
