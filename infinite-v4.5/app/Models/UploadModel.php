...
class UploadModel extends BaseModel
{
        ...
        if (!empty($file) && !empty($file->getName())) {
            ...
            if ($keepOrjName == true) {
                if (file_exists(FCPATH . $directory . '/' . $orjName)) {
                    $orjName = pathinfo($orjName, PATHINFO_FILENAME) . '-' . generateToken(true) . '.' . pathinfo($orjName, PATHINFO_EXTENSION);
                }
                $newName = $orjName;
            }
    ...
    public function uploadPostImage($tempPath, $type)
    {
        ...
        $newPath = 'uploads/images/' . $this->createUploadDirectory('images') . $name . generateToken(true);
    ...
    {
        $newPath = 'uploads/gallery/' . $this->createUploadDirectory('gallery') . 'image_' . $width . 'x_' . generateToken(true);
        $img = Image::make($tempPath)->orientate();
    ...
    public function uploadAvatar($userId, $path)
    {
        ...
        $newPath = 'uploads/profile/' . $directory . 'avatar_' . $userId . '_' . generateToken(true);
