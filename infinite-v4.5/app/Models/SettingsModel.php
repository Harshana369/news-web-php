...
class SettingsModel extends BaseModel
{
    ...
    public function updateGeneralSettings()
    {
        ...
        $logoWidth = clrNum(inputPost('logo_width'));
        $logoHeight = clrNum(inputPost('logo_height'));
        $logoSize = [
            'width' => $logoWidth >= 10 && $logoWidth <= 500 ? $logoWidth : 160,
            'height' => $logoHeight >= 10 && $logoHeight <= 500 ? $logoHeight : 60
        ];
        $data['logo_size'] = serialize($logoSize);
        return $this->builderGeneral->where('id', 1)->update($data);
    }
    ...
    {
        return $this->builder->where('lang_id', clrNum($lang_id))->get()->getRow();
    }
    ...
    {
        $arrayFonts = array();
        $fonts = $this->builderFonts->whereIn('id', [clrNum($settings->primary_font), clrNum($settings->secondary_font)], false)->get()->getResult();
        if (!empty($fonts)) {
    ...
    {
        return $this->db->table('fonts')->where('id', clrNum($id))->get()->getRow();
    }
    ...
    public function editFont($id)
    {
        ...
        if (!empty($font)) {
            ...
            return $this->db->table('fonts')->where('id', clrNum($id))->update($data);
        }
        return false;
    }
    ...
    function deleteOldSessions()
    {
        ...
        $this->db->table('ci_sessions')->where("timestamp < DATE_SUB('" . $now . "', INTERVAL 7 DAY)")->delete();
