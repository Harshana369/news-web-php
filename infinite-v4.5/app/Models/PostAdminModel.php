...
class PostAdminModel extends BaseModel
{
    ...
    public function buildQuery()
    {
        ...
        $this->builder->select("posts.* , users.username as username, users.slug as user_slug, images.image_big, images.image_mid, images.image_small, images.image_mime, images.storage")
            ->join($tblUsers, 'users.id = posts.user_id', 'left')
            ->join($tblImages, 'images.id = posts.image_id', 'left');
    }
    ...
    public function setData()
    {
        ...
        if (!empty(inputPost('image_url'))) {
            ...
            $data['image_id'] = clrNum(inputPost('post_image_id'));
            $data['image_url'] = '';
        }
    ...
    {
        if (!empty($this->builder->where('slug', $slug)->where('id !=', $id)->get()->getRow())) {
            return true;
        }
        return false;
    }
    ...
    //get posts count
    public function getPostsCount($listType)
    {
        $this->filterPosts($listType, true);
        return $this->builder->countAllResults();
    }

    //get paginated posts
    public function getPostsPaginated($perPage, $offset, $listType)
    {
        $this->buildQuery();
        $this->filterPosts($listType);
        if ($listType == 'slider-posts') {
            $this->builder->addUseIndex('idx_posts_slider');
        } elseif ($listType == 'our-picks') {
            $this->builder->addUseIndex('idx_posts_our_picks');
        } elseif ($listType == 'pending-posts') {
            $this->builder->addUseIndex('idx_posts_optimized');
        } elseif ($listType == 'drafts') {
            $this->builder->addUseIndex('idx_status, idx_created_at');
        } else {
    ...
    //filter by values
    public function filterPosts($listType, $countRows = false)
    {
        $postType = inputGet('post_type');
        $langId = clrNum(inputGet('lang_id'));
        $userId = clrNum(inputGet('author'));
        $categoryId = clrNum(inputGet('category'));
        $subCategoryId = clrNum(inputGet('subcategory'));
        $q = trim(inputGet('q') ?? '');
        ...
        if (!empty($listType) && $listType == 'pending-posts') {
            $this->builder->where('posts.visibility', 0)->where('posts.status', 1);
        } elseif (!empty($listType) && $listType == 'drafts') {
            $this->builder->where('posts.status', 0);
        } elseif (!empty($listType) && $listType == 'slider-posts') {
            $this->builder->where('posts.visibility', 1)->where('posts.status', 1)->where('posts.is_slider', 1);
        } elseif (!empty($listType) && $listType == 'our-picks') {
            $this->builder->where('posts.visibility', 1)->where('posts.status', 1)->where('posts.is_picked', 1);
        } else {
            $this->builder->where('posts.visibility', 1)->where('posts.status', 1);
        }
    }
    ...
    {
        return $this->builder->where('category_id', clrNum($categoryId))->where('visibility', 1)->where('status', 1)->countAllResults();
    }
    ...
    public function deleteOldPosts()
    {
        if ($this->generalSettings->auto_post_deletion == 1) {
            $days = clrNum($this->generalSettings->auto_post_deletion_days);
            if ($days > 1) {
    ...
    {
        return $this->db->table('post_images')->where('id', clrNum($fileId))->get()->getRow();
    }
    ...
    public function deletePostAdditionalImage($fileId)
    {
        ...
        if (!empty($image)) {
            $this->db->table('post_images')->where('id', clrNum($fileId))->delete();
        }
    }
    ...
    {
        return $this->db->table('post_files')->where('id', clrNum($fileId))->get()->getRow();
    }
