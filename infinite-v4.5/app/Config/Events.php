...
Events::on(
    ...
    static function (\CodeIgniter\Database\Query $query) {
        ...
        if (!empty($lowerSql)) {
            ...
            $tablesAffected = ['ad_spaces','categories','fonts','general_settings','languages','language_translations','pages','polls','roles_permissions','settings'];
            foreach ($tablesAffected as $table) {
                if (strpos($lowerSql, $table) !== false && in_array($operation, ['INSERT', 'UPDATE', 'DELETE'])) {
                    resetCacheData('static', false);
                }
            }
        }
    }
