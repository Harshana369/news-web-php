...
class AuthController extends BaseController
{
    ...
    public function forgotPasswordPost()
    {
        ...
        if (empty($user)) {
            ...
            setSuccessMessage("msg_reset_password_success");
        }
    ...
    public function resetPassword()
    {
        ...
        $data["user"] = $this->authModel->getUserByResetToken($token);
    ...
    {
        $val = \Config\Services::validation();
        $val->setRule('password', trans("password"), 'required|min_length[4]|max_length[250]');
        $val->setRule('confirm_password', trans("confirm_password"), 'required|matches[password]');
        if (!$this->validate(getValRules($val))) {
            $this->session->setFlashdata('errors', $val->getErrors());
            return redirect()->back()->withInput();
        } else {
            $token = inputPost('reset_token');
