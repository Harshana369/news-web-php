<?php
$socialArray = getSocialLinksArray($settings);
$socialLinks = [];
...
if (!empty($postJsonLD)) {
    ...
    $newsArticleSchema = [
        ...
        "author" => [
            "@type" => "Person",
            "name" => escMeta($postJsonLD->username),
            "url" => base_url("profile/" . $postJsonLD->user_slug)
        ],
        "publisher" => [
            "@type" => "Organization",
            "name" => clrQuotes($settings->application_name),
            "logo" => [
                "@type" => "ImageObject",
                "width" => getLogoSize('width'),
                "height" => getLogoSize('height'),
